function StartupAssistant()
{
    ...
    this.firstMessage = $L('Here are some tips for first-timers:<ul><li>SkyNet has no tips yet</li></ul>');
	
    this.secondMessage = $L('We hope you enjoy being able to manage your network of devices.<br>Please consider making a <a href=http://www.webos-internals.org/wiki/WebOS_Internals:Site_support>donation</a> if you wish to show your appreciation.');
	
    // on new version start
    this.newMessages =
	[
	 // Don't forget the comma on all but the last entry
	 { version: '0.1.0', log: [ 'Initial skeleton version' ] }
	 ];
...
StartupAssistant.prototype.setup = function()
{
    ...
    // set title
    if (vers.isFirst) {
	this.titleContainer.innerHTML = $L('Welcome To SkyNet');
    }
    else if (vers.isNew) {
	this.titleContainer.innerHTML = $L('SkyNet Changelog');
    }
